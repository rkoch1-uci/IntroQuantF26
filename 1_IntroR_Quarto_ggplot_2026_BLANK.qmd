...
author: "Ryan Koch"
last modified: "Sept 24, 2026 @ 4:14 PM"
format: html
editor: source
---
...
THIS TUTORIAL COMPLETED BY: (Ryan Koch)
...
ggplot(candy_rankings) + 
  aes(
    x = chocolate) # this says what to put on the axes
```
...
```{r}
price_of_fruity = ggplot(candy_rankings) +
...
```{r}
sugar_vs_win = ggplot(candy_rankings) +
  aes(x = sugarpercent, y = winpercent) +
  geom_point() +
  geom_smooth()
sugar_vs_win
...
ggplot(candy_rankings) +  
  aes(x = chocolate) + 
  geom_bar(color = 'chocolate4', fill = "white") 
```
...
```{r}
ggplot(candy_rankings) +  
  aes(x = chocolate, fill = chocolate) + 
  #Figure out whatever the position function means on Thurs 10/1
  geom_bar(position = ) 
```
